...
public function readFood(Request $request)
{
    ...
    public function findFood(Request $request)
    {
        ...
        $foods = Food::where('food_name', 'like', '%' . $food_name . '%')->paginate(4); // Cho phép khớp một phần và phân trang kết quả

        if ($foods->isEmpty()) {
            return redirect()->route('food.list')->with('error', 'Không tìm thấy món ăn.');
        }
